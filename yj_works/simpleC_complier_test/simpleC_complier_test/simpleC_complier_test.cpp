...
int run()
{
	...
	{
		i = *pc++;
		if (i == LEA)						// LEA <offset>:用于获取函数调用中传入的参数
		{
			ax = (int)(bp + *pc++);
		}
		else if (i == IMM)
		...
		else if (i == ENT)						// 保存当前栈底指针并为函数调用栈安排空间
		{
			*--sp = (int)bp;
			bp = sp;
			sp = sp - *pc++;
		...
		else if (i == LEV)						// 做销毁函数栈的扫尾工作,恢复栈底指针等,对应汇编中 ret 指令
		{
			sp = bp;
			bp = (int *)*sp++;
			pc = (int *)*sp++;
...
void trace_dump(int *data, int len)
{
	//
	int n = data[3];
	printf("%2.2x", n);
...
//
void trace_char_dump(char *data, int len)
{
	int i = 0;
	while (i < len)
	{
...
int main(int argc, char **argv)
{
	...
	// 将支持的关键字和库函数转换为特定结构写入到 symbol 所对应的内存区域中，方便之后解析的时候使用 - add by tianyu 202012.28
	...
	//
	*--sp = EXIT;		// call exit if main return
	*--sp = PUSH;
	temp = sp;
	*--sp = argc;
	*--sp = (int)argv;
