...
// =========================== ItemRect ===============
//
QRectF ItemRect::boundingRect() const  //静态函数
{
...
bool ItemRect::loadXml(QXmlStreamReader *xml)
{
    int x = xml->attributes().value("x").toInt();
    int y = xml->attributes().value("y").toInt();
...
//
QString ItemRect::getID()
{
    return m_strName;
}

//
void ItemRect::setBackColor(int color)
{
    m_color = color;
    this->update();
}
...
//
bool ItemRect::saveXml(QXmlStreamWriter * xml)
{
    //
    xml->writeStartElement(tr("rect"));
    xml->writeAttribute(tr("x"),QString("%1").arg(pos().x()));
